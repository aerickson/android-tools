...
# Run the BIOS/CPU probe concurrently and retry hosts that are temporarily
# unavailable (for example, while rebooting). Each attempt keeps its own output
# and error files so a failed SSH attempt never overwrites a successful result.
...
probe_script="$script_dir/../ct_scripts/bios_and_cpu_speed_probe.sh"
run_dir="$script_dir/pssh_bios_cpu_speed_probe_$(date +%Y%m%d-%H%M%S)"
...
# parallel-ssh permits comments, but normalize the file for the progress lists.
